...
- 인지 강건성 순위: learned detector **−13.9pp** > dropout 0.3 **−12.7pp**
  ≫ latency 0.1 s **−2.5pp**(보정 후) > range error ±0.3 m **≈0pp**
- 실기 latency 예산: **0.2 s 목표 / 0.3 s 상한** (10 Hz 제어 기준 2~3스텝)
...
- captured **0.796 → 0.067 붕괴**. 원인 = 안전항이 개방공간에서 **+1.386/step 고정
  수입**(배회 가치 104 ≫ 캡처 가치 30). 재베이스라인 + 충돌 −20 + PBRS로 회복.
...
- clean 80.54/17.17 기준: dropout 0.3 **−12.7pp**, latency 0.1 s **−42.7pp** /
  0.2 s −62.0pp, range error ±0.15/0.30 m **≈0pp**, learned detector −13.9pp.
...
- **R3 latency 판정 공식 기각**: 진짜 비용은 0.1 s −2.5pp / 0.2 s −3.9 / 0.3 s −8.0 /
  0.5 s −15.8pp. **실기 latency 예산 0.2 s 목표 / 0.3 s 상한.** P3 기본 ON 승격.

**08-06~07 · dropout 채널 사냥 (−12.7pp의 해부)**
- dropout과 learned detector는 **실패 경로가 정반대**: dropout은 표적에 접근하되 좁은
...
**08-10 · H4 플래그 검정 (진행 중 → 결과는 아래 §3 참조)**
- 가설: 해악은 상태 보정이 아니라 correct()의 age 리셋 + fused_visible OR — 예측을
  되먹인 측정에 "보인다/방금 봤다"를 통보하는 **플래그 오염**.
...
7. **밀도×속도 맵**: 밀도 −78pp vs 속도 −4.2pp — "밀도가 거의 전부" (07-30, v2 재측정 예정)
...
13. **★ R3 대반전**: "latency −42.7pp"는 ego-motion 아티팩트 — 94% 회수, 진짜 −2.5pp;
    실기 예산 0.2 s/0.3 s (08-06)
14. **기각의 과학**: 그럴듯한 중간 변수 6개(P0/P1/P2/backfill/H3/게이트)를 저비용 A/B로
    연쇄 기각 — 유일한 생존자 H2 +3.26pp 2시드 (08-05~07)
15. **다음 관문**: learned detector −13.9pp — 센서 전용 주장의 마지막 관문 (08-06)
...
| **learned detector** | **66.62%** | **−13.9pp** |

**남은 일 (발표의 "향후 계획" 슬라이드)**
1. **learned detector 격차 −13.9pp 해소** — 현 1순위. 실패 서명이 "표적 근처 미도달"
   이므로 검출 빈도(14.25% vs 21.21%)·거리 의존성부터 진단.
2. dropout 잔여 ~70% 채널 규명 (H4 플래그 검정 결과에 따라 방향 결정).
3. **v2 밀도×속도 맵 재측정** — 논문 헤드라인 그림을 v2 계약으로.
4. corridor B-arm(946-D) 재평가, R4 temporal fusion(P3 통과 시 전제 충족됨).
