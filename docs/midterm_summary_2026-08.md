...
- **v2 밀도×속도 맵**(20셀): 밀도 축 **−11.4pp** vs 속도 축 **−2.7pp** — 비대칭의 형태는
  v1에서 재현. 단위 밀도당 기울기는 v2 **2.02** vs v1 **2.98pp**로 같은 자릿수다
  (v1의 −78pp는 밀도 범위가 6배 넓었던 탓 — **숫자가 아니라 기울기로 비교**).
- **새 발견 — 속도×밀도 상호작용**: 표적이 0.3→1.5 m/s로 빨라질 때의 비용이
  130막대 **−0.88pp** → 205막대 −3.06pp → 220막대 **−5.99pp**로 커진다.
  즉 "표적 속도는 난이도 축이 아니다"는 **저밀도에서만 참**이며, 논문 서술은
  **"속도는 밀도와 곱해져서만 유의미해진다"**로 바뀌어야 한다. v1은 밀도를 평균 내어
  이 항을 볼 수 없었다.
...
7. **밀도×속도 맵**: v1 밀도 −78pp vs 속도 −4.2pp → **v2에서 형태 재현**(−11.4 vs −2.7pp,
   단위밀도당 2.02 vs 2.98pp)이되 **속도 비용이 밀도와 함께 커지는 상호작용** 발견
   (130막대 −0.88pp → 220막대 −5.99pp) (07-30 / 08-10)
...
14b. **★ 채널 분해**: 플래그만 차단해도 H2 이득의 69% 회수 → 손해의 2/3는 "봤다"는 거짓
    증언, 1/3은 상태 오염. 정책이 읽는 건 공분산이 아니라 이산 신호 (08-10)
...
2. dropout 잔여 ~70% 채널 규명 — LiDAR 연관 경로(최대 26%)는 H2/H3/게이트/H4로 내부까지
   분해 완료했으므로, 남은 손실은 그 **경로 바깥**에 있다.
3. ~~v2 밀도×속도 맵 재측정~~ — **완료(08-10)**. 다음은 이 맵을 논문 그림으로 정리.
