...
- **확정 — 원호 기하(`dwa_arc`)의 우위는 평가 시드를 넘어 재현된다.** ep25000 계보, 평가 시드
  ...
  결합 추정은 riskcap 대비 **−1.49 pp [−1.90, −1.08]**(Cochran Q 10.3, df 14, I² 0 %),
  정지법칙 대비 −1.25 pp [−1.65, −0.85]다. 사전등록 예측 P1·P2·P4 통과, P3는 6 중 5.
- **기각 — P5 "정지법칙과 riskcap은 모든 밀도에서 동등".** 70 bars에서 정지법칙이 **−1.19 pp
  [−2.01, −0.37]** 낫고(세 시드 부호 일치) 100 bars 이상에서 사라진다. 전체 결합은 0을 포함한다
  (−0.23 [−0.65, +0.20]). C3는 **밀도 조건부**로만 쓴다.
...
- **소스 무결성**: 결합에 쓴 셀은 두 동결 트리에서 나왔으나(`b51c3d2f` / `3aa30cd8`), seed 523의
  205 bars 원호 셀을 새 트리에서 재실행해 **비트 동일**(306/1699/44 of 2049)을 확인했다.
