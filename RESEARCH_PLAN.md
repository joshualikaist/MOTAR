...
- `navrl_bars_env.py` (env_config): **24×24×3m** 아레나 + 정적 기둥(bars) **48개**. Phase 1 실험 당시에는 지터드 격자, 현재 Phase 2는 **NavRL식 랜덤 rejection 배치**(배치 밴드 x∈[3.1,23]×y∈[0,24]≈**478 m²** → 48개 기준 ~10개/100m²)를 사용. **2D 비행 @ 고도 1m**. **스폰: 드론 x≈0(왼쪽 가장자리) → 목표 x=k(먼쪽) = 매 에피소드 막대밭 전체 관통**(옛 방사형/18m 설계 폐기). 커리큘럼: k_max 7→24m 램프 후 full-scale hold, 이어 k_min 5→20m. 드론 충돌 = **0.28m 박스**(팔/프롭 포함, navrl 전용 URDF).
...
- **완료 기준 M1(성공률 ≥ 90%) → ✅ 달성**: 새 스킴(cross-field 24m·당시 지터드 격자막대·0.28m 박스충돌) 진행 = run `1904` 정직 baseline **0.65** → crash 레버 Run D `2207` **0.66**(감속만, 리워드로 안 줄어드는 **기하 바닥** 발견) → **yaw 제어(option b)로 captured 0.954 / crash 0.046**(run `ppo_260715_0251`, k_min=20 최심에서도 0.95). **NavRL 0.81·M1 0.90 모두 상회.** 잔여 crash 원인 = 요-고정 ±30° corner-clip(대각 footprint 0.40m)이었고 yaw 권한(action 3→4)으로 해결. 대조군 브랜치 `ablation/yaw-off`(논문 attribution용). 상세: `WORKLOG.md`.

### Phase 2 — 정적 밀도 스윕 (1주) → RQ1 전반부 ← **현재 여기**
- 밀도 정의: **개수/100m²**, 분모는 **막대 배치 밴드 ≈478 m²**(x∈[3.1,23]×y∈[0,24]; 스폰 회랑엔 막대 없음). NavRL 기준 350개/1600m² ≈ **22개/100m²** — 이전 계획의 "2.2"는 10배 오타. (주의: 전체 아레나 576 m²로 나눈 옛 표기 8.3/26 등은 폐기 — 밴드 분모로 통일.)
- 밀도 5레벨: {5, 10, 15, 22, 30}개/100m² (NavRL 밀도 22를 중심으로) = 막대수 **{25, 50, 75, 110, 150}개**(밴드 478 m² 기준 {5.2, 10.5, 15.7, 23.0, 31.4}). **VRAM: 150막대+256env≈6.1GB로 8GB 안 → num_envs 256 유지 가능**(고밀도에서 128로 낮출 필요 없음 — 스크리닝으로 확인).
...
- **진행(2026-07-15)**: 밀도 배관 완료(`NAVRL_MAX_BARS`/`NAVRL_NUM_BARS`, `keep_in_env=False`로 런타임 활성수
  제어; 커밋 `7e1b6a8`+`0aed4be`). 실제 sweep = 막대수 {25, 50, 75, 110, 150}개 = {5.2, 10.5, 15.7, 23.0, 31.4}개/100m²(밴드 478 m²).
  VRAM 스크리닝: 150막대+256env ≈ 6.1GB → num_envs 256 그대로 가능.
- **⚠️ 배치 방식 전환(커밋 `54d7cfe`+`10708e2`)**: 지터드 격자 → **NavRL식 랜덤 rejection**으로 전환(격자 행/열
  아티팩트 제거 + 사양 충실도). 이 때문에 **옛 격자 run은 새 랜덤 스윕과 비교 불가**: run `1552`(150막대, 격자,
  captured 0.85)·`1922`(75막대, 격자)의 summaries는 폐기(삭제됨). **밀도 스윕 전체를 랜덤 배치로 재실행** 중 —
  run `2038`(150막대, 랜덤, seed 1)이 새 기준 150막대 재-baseline. **랜덤 스윕 완료(2026-07-16, seed 1)**: 25/50/75/110/150막대 → captured **0.97/0.97/0.94/0.93/0.66** — NavRL 앵커(110)까지 평탄, 110→150에서 절벽(−27pt). 기하 분석: 배치 밴드 478m²의 RSA 재밍 한계 ≈148막대, 완화 시작 ≈115-120막대 → 절벽은 배치 알고리즘의 기하 한계와 일치. ⚠️ 25/50은 4GB 머신에서 minibatch 정합(4096) 이전에 학습된 confound 있음(정성 결론 유효, 정량 인용 시 주의). 잔여: 150 seed2(재현성), 120막대(절벽 경계), 25/50 재실행(confound 해소) — 진행/예약.
...
- 관측 **156** 유지(**zero-shot 가능**)/옵션 표적속도(3) 추가 시 **159**(재학습). 평가 (a)**zero-shot**(P1/P2 정책 그대로) vs (b)**재학습**.
- **구현 완료(2026-07-17)**: `_advance_target`(cv반사/waypoint/circle, rl_dt=0.1s, 막대 push-out+벽반사), range-rate 리워드, **PBRS re-anchor**(표적 이동분을 드론 상벌에서 제거), **상대프레임 세그먼트 캡처**(터널링 방지, v=0에서 point 테스트의 상위집합), 속도 커리큘럼(0→final, epoch비례, checkpoint 생존). 노브: `NAVRL_TARGET_SPEED_FINAL`(학습)/`NAVRL_TARGET_SPEED`+`NAVRL_TARGET_PATTERN`(평가 셀)/`NAVRL_SEED`(평가 재현성). 상세 계획: `PHASE3_PLAN.md`.
...
- **S_dyn**: N_d=5 최근접, 장애물당 10차원[상대위치단위(3)+2D거리+z거리+상대속도(3)+폭·높이 카테고리], zero-pad → **관측 156→206**. 리워드 `+ r_ds`(재베이스라인 log거리, loiter income 제거). 종료 `|= 해석적 동적충돌`.
