...
GAMES="${2:-2049}"
# Same per-area density schedule as the v1 sweep, scaled to the 1328 m^2 v2 band:
# 70/150/210/280 bars = 5.3 / 11.3 / 15.8 / 21.1 per 100 m^2.
...
if missing:
    print(
        "[eval_v2] REFUSING: checkpoint has no v2 arena provenance (%s).\n"
...
if bad:
    print("[eval_v2] REFUSING: arena contract mismatch:\n  " + "\n  ".join(bad), file=sys.stderr)
    sys.exit(2)
print(
...
if [[ "${NAVRL_V2_FORCE:-0}" == "1" ]]; then
    echo "[eval_v2] NAVRL_V2_FORCE=1 -- provenance gate bypassed by request."
fi
...
for N in ${DENSITIES}; do
    echo "======== v2 density ${N} bars ($(python3 -c "print(f'{${N}/1328*100:.1f}')")/100m2) ========"
