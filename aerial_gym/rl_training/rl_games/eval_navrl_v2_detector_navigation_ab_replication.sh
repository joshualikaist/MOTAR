...
#
# CONFIRMATORY REPLICATION of the Gate 3 stage-B navigation non-inferiority result.
#
# The original (seeds 83/89) passed with a CI lower bound of -1.790 pp against the -2.0 pp
# margin -- 0.21 pp of headroom. Preregistered here BEFORE any replication data exists:
#   - fresh unused seeds 97/101, everything else byte-identical to the original campaign;
#   - the SAME -2.0 pp margin on the two-seed pooled learned-analytic capture difference;
#   - reported SEPARATELY from the original; the two are never pooled post hoc.
#
...
RESULT_ROOT="${NAVRL_DETECTOR_AB_REPL_RESULT_ROOT:-${REPO_ROOT}/results/navrl_v2_detector_navigation_ab_replication_seed97_101_schema2}"
...
if [[ "${PREFLIGHT}" == "1" ]]; then
    ...
    echo "[det-ab] PREFLIGHT PASS | 4 cells seeds=97,101 bars=205 margin=-2pp"
    exit 0
fi
