"""Full-evidence page contracts (docs/status/evidence.html) plus the browser-preview DOM on the project page.

The detailed paper page moved from index.html to evidence.html on 2026-09-24 when index.html became the
concise project page; its contracts moved with it. Project-page contracts are in test_project_page.py.
"""
import hashlib
...
class ResearchOverviewTest(unittest.TestCase):
    def setUp(self):
        self.text = (SITE / 'evidence.html').read_text()
        self.index = (SITE / 'index.html').read_text()
    ...
    def test_paper_structure_and_accessibility(self):
        ...
        # Numbered figures 1 and 3-9 plus Figure T1 (drawn in CSS, no image asset). Figure 2 was
        # the interactive canvas; it now lives on the project page and this page points to it.
        # Figure 9 is the quantitative positioning diagram generated by
        # tools/build_quantitative_positioning_figure.py from the registry.
        ...
        self.assertIn('index.html#demo', self.text)
    ...
    def test_all_active_local_links_and_html_fragments(self):
        for page_name, page in (('evidence.html', self.page), ('index.html', Page(self.index))):
            for link in page.links:
                parsed = urlsplit(link)
                if parsed.scheme or parsed.netloc:
                    continue
                target = (SITE / unquote(parsed.path or page_name)).resolve()
                self.assertIn(ROOT, target.parents, link)
                self.assertTrue(target.exists(), link)
                if parsed.fragment and target.suffix == '.html':
                    self.assertIn(unquote(parsed.fragment), Page(target.read_text()).ids,
                                  page_name + ' -> ' + link)
    ...
    def test_pages_landing_links_do_not_escape_the_docs_root(self):
        ...
        for text in (self.text, self.index):
            self.assertNotIn('href="../../', text)
            self.assertIn('https://github.com/joshualikaist/MOTAR-public"', text)
            self.assertIn('MOTAR-public/blob/main/CITATION.cff', text)
            self.assertNotIn('https://github.com/joshualikaist/MOTAR"', text)
        prefix = 'https://github.com/joshualikaist/MOTAR-public/blob/main/'
        for link in self.page.links + Page(self.index).links:
            if link.startswith(prefix):
                self.assertTrue((ROOT / link[len(prefix):]).exists(), link)

    def test_viewer_dom_and_script_order_preserved(self):
        # The frozen browser preview (GT_BROWSER_EPISODE_V1) is embedded on the project page.
        ...
        for name in archive.ids:
            if name == 'stage' or name.startswith(('hud-','sl-','lbl-','btn-','cb-','sel-')):
                self.assertIn(name, index.ids)
        self.assertIn('id="motion-mode-note"', self.index)
        def strip_ver(src):
            return src.split('?')[0]
        current = [s for s in index.scripts if 'status_manifest' not in s]
        ...
        self.assertIn('value="gt-free-roam" selected', self.index)
        self.assertIn('value="gt-route-track" selected', self.index)
        self.assertIn('value="routed-preview"', self.index)
        self.assertIn('BROWSER GT PREVIEW', self.index)
        self.assertIn('NOT PPO · NOT PHYSX · NOT RESEARCH EVIDENCE', self.index)
        self.assertIn('NOT PhysX/PPO', self.index)
        self.assertIn('browser-only visualization', self.index.lower())
        # The detailed page no longer loads the WebGL stack.
        self.assertFalse(any('three.min.js' in s for s in self.page.scripts))
    ...
    def test_target_behavior_ladder_is_explicit_and_fail_closed(self):
        ...
        # Corrected 2026-09-24: the frozen-policy H/E0/E1/E2 comparison ran (2026-09-18/19);
        # the fail-closed boundary that still holds is that TM-E3/TM-E4 have no result.
        self.assertIn('TM-E3 and TM-E4 have no implementation or result', self.text)
        self.assertIn('target_motion_generalization_2026-09-19.md', self.text)
        self.assertNotIn('No E0/E1/E2 policy-performance grid is reported', self.text)
