...
| 대비 (crash, pp) | 평가 시드 3 × 밀도 5 결합 | 이질성 I² |
|---|---|---:|
| 원호 − riskcap | **−1.49 [−1.90, −1.08]** | 0 % |
| 원호 − 정지법칙 | **−1.25 [−1.65, −0.85]** | 15 % |
| 정지법칙 − riskcap | −0.23 [−0.65, +0.20] | 29 % |

원호는 15개 (시드, 밀도) 조합 **전부**에서 crash가 최저였고, 시드 간 이질성이 0 %입니다.
동일 조건을 서로 다른 소스 트리에서 재실행해 **비트 단위로 같은 결과**임을 확인했습니다
(crash 306 / captured 1699 / timeout 44 of 2049).
...
1,000 epoch 더 학습하면 그 이득이 사라집니다(`+0.48 pp [−1.82, +2.78]`). 학습 루프의 필터 하나만
